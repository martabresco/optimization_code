...
class Optimal_Investment:
    ...
    # Constraint for line power flows based on voltage angle differences and line reactance
        ...
        # self.constraints.eta_not_zero={
        #     k: self.model.addConstr( self.variables.eta[k] >=0) for k in self.data.demand_scenarios.columns
        #     }
        
                    # Risk Constraints
        self.constraints.eta_positive = {
            k: self.model.addConstr(
                self.variables.eta[k] >= 0,
                name=f"eta_positive_scenario_{k}"
            )
            for k in self.data.demand_scenarios.columns
        }
...
#         # Risk parameters
    ...
    def display_results(self):
        ...
        if hasattr(self.variables, "eta"):
            for scenario in self.data.demand_scenarios.columns:
                if self.variables.eta[scenario].x >= 0:
                    print(f"  Scenario {scenario}: Eta = {self.variables.eta[scenario].x:.2f}")
        else:
...
#     model_instance = Optimal_Investment(input_data=input_data, complementarity_method='SOS1',K=5e8)
